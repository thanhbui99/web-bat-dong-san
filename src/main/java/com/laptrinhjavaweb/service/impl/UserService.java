...
public class UserService implements IUserService {
    ...
    @Override
    public List<StaffResponse> getStaffByBuildingId(Long buildingId) {
        try {
            List<StaffResponse> staffResponses = new ArrayList<>();
            userRepository.findByStatusAndRoles_Code(1, "STAFF").stream().forEach(userEntity -> {
                StaffResponse staff = StaffResponseConverter.convertStaff(userEntity);
                if (userRepository.existsByIdAndBuildings_Id(userEntity.getId(), buildingId)) {
                    staff.setChecked("checked");
                }
                staffResponses.add(staff);
            });
